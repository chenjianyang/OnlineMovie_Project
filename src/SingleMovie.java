...
public class SingleMovie extends HttpServlet {
   ...
   // private DataSource dataSource;
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        System.out.println("in the single movie page");
        ...
        try {
           ...
           // String query9 = "select movies.id, movies.price, movies.title, movies.year, movies.director, group_concat(distinct genres.name SEPARATOR ',') as genres , ratings.rating from movies, genres, genres_in_movies, ratings where movies.id = ratings.movieId and movies.id = genres_in_movies.movieId and genres_in_movies.genreId = genres.id and movies.id = ?";
            String query="select newa.price,newa.id, newa.title, newa.year, newa.director, substring_index(group_concat(distinct genres.name SEPARATOR ','), ',', 3) as genres, newa.rating\n" +
                    "                                from (select *\n" +
                    "                                from movies  left join ratings\n" +
                    "                                on movies.id=movieId) as newa,genres, genres_in_movies\n" +
                    "                                where newa.id=genres_in_movies.movieId and genres.id=genres_in_movies.genreId and newa.id=? group by newa.id ";
            ...
            System.out.println("single check1");
            // Assign static data
            while (rs.next()) {
                ...
                System.out.println("single check2");
            ...
            statement2.close();
            dbcon.close();
            System.out.println("no erroro in the singlemovie");
        } catch (Exception e) {
